...
public class OrderPayController extends BaseController {
    ...
    public ResponseObj doPay(@Valid @RequestBody PayForm form, BindingResult result) {
        ...
        if (orderInfo == null) {
            return ResponseObj.fail(StatusCode.BIZ_FAILED, "订单记录不存在");
        }
        ...
        if (channel == null || !channel.isThirdParty()) {
            return ResponseObj.fail(StatusCode.BIZ_FAILED, "支付渠道不支持");
        }
        ...
        if (paymentService.addPayment(payment) != null) {
            ...
            return ResponseObj.success(payment, new HashMap<String, Object>(1) {{
